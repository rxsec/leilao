...
export default async function CadastroPorAdminPage({
  params,
}: {
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#eef4f8_0%,#f7f8fa_45%,#f8f5ef_100%)] px-4 py-10 sm:px-6">
      <section className="mx-auto max-w-xl rounded-[1.4rem] border border-[#d5e0e8] bg-white p-6 shadow-[0_18px_60px_rgba(17,17,17,0.08)] sm:p-8">
        ...
        <p className="mt-3 text-sm leading-7 text-neutral-600">
          Este cadastro sera vinculado a carteira administrada por {admin.name}.
        </p>
        <div className="mt-6">
          <AuthForm
            mode="register"
            registrationSlug={admin.registrationSlug}
            helperText={
              <>
                Depois do cadastro, seu atendimento e seus arremates ficarao
                vinculados ao painel administrativo de <strong>{admin.name}</strong>.
              </>
            }
          />
        </div>
